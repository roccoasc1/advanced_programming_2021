...
class stack_pool{
    ...
    using iterator = _iterator<N, T>;
    using const_iterator = _iterator<N, const T>;
...
class _iterator{
    ST current;
   	std::unique_ptr<stack_pool<O,ST>> ps;
    public:
    using value_type = O;
    using reference = value_type&;
    using pointer = value_type*;
    using difference_type = ST;
    using iterator_category = std::forward_iterator_tag;
     
    _iterator(ST x, stack_pool<O,ST> pp): current{x}, ps{std::make_unique<stack_pool<O,ST>>(pp)} { std::cout << "ctor\n";}
    //_iterator(const _iterator& x): current{x.current}, sp{x.sp}{}
    _iterator(_iterator&& x ): current{std::move(x.current)}, ps{std::move(x.ps)}{}
    //_iterator() = default;
    ...
    reference operator*() const { 
    	//std::cout << "*  "<< ps->value(current) << std::endl;
    	return (*ps).value(current); 
    } 
    
    pointer operator->(){ return &**this; }

    _iterator& operator++(){
        current = ps->next(current);
